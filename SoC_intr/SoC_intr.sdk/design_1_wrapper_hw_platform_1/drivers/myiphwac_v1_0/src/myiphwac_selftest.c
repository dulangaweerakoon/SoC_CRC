...
XStatus MYIPHWAC_Reg_SelfTest(void * baseaddr_p)
{
	...
	for (write_loop_index = 0 ; write_loop_index < 4; write_loop_index++)
	  MYIPHWAC_mWriteReg (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
	for (read_loop_index = 0 ; read_loop_index < 4; read_loop_index++)
	  if ( MYIPHWAC_mReadReg (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
...
XStatus MYIPHWAC_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  MYIPHWAC_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = MYIPHWAC_mReadMemory(baseaddr+4*Index);
